# 🚀 Guia de Deploy - VendaPop em VPS (Sem Docker)

Este guia detalha a instalação completa do VendaPop em uma VPS Ubuntu/Debian sem Docker, incluindo todas as dependências, configurações e SSL gratuito.
...
- Domínio apontado para o IP da VPS (vendapop.com.br)
...
sudo chown -R www-data:www-data /var/www/vendapop
sudo chmod -R 755 /var/www/vendapop
```
...
```bash
# Editar arquivo .env
sudo nano /var/www/vendapop/backend/.env
```
...
MAIL_PORT=587
MAIL_USERNAME=seu-email@vendapop.com.br
MAIL_PASSWORD=SUA_SENHA_EMAIL_AQUI
MAIL_ENCRYPTION=tls
MAIL_FROM_ADDRESS="noreply@vendapop.com.br"
MAIL_FROM_NAME="${APP_NAME}"

FRONTEND_URL=https://vendapop.com.br
...
- **Configurar reCAPTCHA v3:**
  ...
  3. Adicione os domínios: `vendapop.com.br` e `api.vendapop.com.br`
...
```env
# URL da API (subdomínio api.vendapop.com.br)
VITE_API_BASE_URL=https://api.vendapop.com.br/api
...
1. Acesse: https://www.google.com/recaptcha/admin/create
2. Selecione "reCAPTCHA v3"
3. Adicione o domínio: `vendapop.com.br`
4. Copie a **Site Key** e cole em `VITE_RECAPTCHA_SITE_KEY`
5. Copie a **Secret Key** e configure no backend (ver seção Backend)

**⚠️ NOTA:** Se você usa subdomínio separado para API (`api.vendapop.com.br`), certifique-se de adicionar ambos os domínios no reCAPTCHA:
- `vendapop.com.br`
- `api.vendapop.com.br` (se necessário)
...
# Dar permissões corretas
sudo chown -R www-data:www-data /var/www/vendapop/frontend/dist
```
...
server {
    listen 80;
    server_name api.vendapop.com.br;
    root /var/www/vendapop/backend/public;
...
server {
    listen 80;
    server_name vendapop.com.br www.vendapop.com.br;
    root /var/www/vendapop/frontend/dist;
...
sudo certbot --nginx -d vendapop.com.br -d www.vendapop.com.br

# Obter certificado SSL para subdomínio da API (se usar)
sudo certbot --nginx -d api.vendapop.com.br
...
```ini
[Unit]
Description=VendaPop API Service
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/var/www/vendapop/backend
...
1. Acesse `https://vendapop.com.br` no navegador
2. Acesse `https://vendapop.com.br/admin/login` para o painel admin
...
# Logs do Laravel
sudo tail -f /var/www/vendapop/backend/storage/logs/laravel.log
...
sudo chown -R www-data:www-data /var/www/vendapop
sudo chmod -R 755 /var/www/vendapop
sudo chmod -R 775 /var/www/vendapop/backend/storage
sudo chmod -R 775 /var/www/vendapop/backend/bootstrap/cache
```
...
**🎉 Parabéns! Seu VendaPop está no ar!**

Acesse: `https://vendapop.com.br`
